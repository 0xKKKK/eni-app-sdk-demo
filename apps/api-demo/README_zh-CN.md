...
`gas-exchange` 默认是 dry-run，只打印 calldata，不会发交易。
...
ERC20 兑换场景需要先允许 `GasExchange` 使用用户的 `fromToken`，常见有两种方式：
...
主流程代码只做一件事：调用 lib 生成 `approve` 和 `exchange` 两步交易。项目方接入时把 `transactions` 交给自己的钱包层按顺序发送即可。
...
核心代码在 [src/lib/gas-exchange.ts](./src/lib/gas-exchange.ts) 和 [src/gas-exchange-demo.ts](./src/gas-exchange-demo.ts)。
